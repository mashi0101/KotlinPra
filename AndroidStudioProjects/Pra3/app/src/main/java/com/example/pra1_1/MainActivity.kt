...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_mora.setOnClickListener{
            ...
            //取出ed_name中的文字作為玩家姓名並用變數儲存
            val playerName = ed_name.text
            //亂數取出0~1不含1的小數，將其*3後得到0~2，取整數作為電腦的出拳
            val comMora = (Math.random()*3).toInt()
            //將玩家出拳結果對應成字串並用變數儲存
            val playerMora = when{
                btn_scissor.isChecked -> 0
                btn_stone.isChecked -> 1
                else -> 2
            }
            ...
            when{
                (playerMora-comMora) == -2 ||
                (playerMora-comMora) == 1 -> {
                    tv_winner.text = "勝利者\n$playerName"
                    tv_text.text = "你超強恭喜你獲勝了"
                }
                (playerMora-comMora) == 0 -> {
                    tv_winner.text = "獲勝者\n平手"
                    tv_text.text = "平局，請再試一次"
                }
                else -> {
